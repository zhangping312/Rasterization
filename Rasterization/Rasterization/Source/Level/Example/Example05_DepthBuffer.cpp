...
namespace Example::Example05
{
	...
	void DepthBuffer::Initialize()
	{
		...
		Vertex vertices[] =
		{
			...
			Vector3(-0.5f, -0.5f,  1.0f), Vector4(0.0f, 1.0f, 0.0f, 1.0f),
			Vector3( 0.0f,  0.5f,  1.0f), Vector4(0.0f, 1.0f, 0.0f, 1.0f),
			Vector3( 0.5f, -0.5f,  1.0f), Vector4(0.0f, 1.0f, 0.0f, 1.0f),
		};
	...
	void DepthBuffer::GUI()
	{
		ImGui::Begin(GetName().c_str());
		ImGui::ColorEdit4("Clear Color", m_ClearColor.values);
		for (int i = 0; i < (int)m_Vertices.size(); ++i)
		{
			std::string poststr = "Position " + std::to_string(i);
			ImGui::DragFloat3(poststr.c_str(), m_Vertices[i].position.values, 0.01f, -1.0f, 1.0f);
			std::string colortstr = "Color " + std::to_string(i);
			ImGui::ColorEdit4(colortstr.c_str(), m_Vertices[i].color.values);
		}

		ImGui::End();
	}

	void DepthBuffer::OnWindowResize(const FW::Event::WindowResizeEvent& event)
	{
