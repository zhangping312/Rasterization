...
namespace Example::Example07
{
	...
	void Camera::Render()
	{
		...
		for (int i = 0; i < (int)(m_Indices.size() / 3); ++i)
		{
			...
			v0.position = Matrix4::Transform(v0.position, MV);
			v1.position = Matrix4::Transform(v1.position, MV);
			v2.position = Matrix4::Transform(v2.position, MV);
	...
	void Camera::GUI()
	{
		...
		{
			ImGui::DragFloat3("Scale##Cube", m_Scale.values, 0.01f, -1.0f, 1.0f);
			ImGui::DragFloat4("Rotation##Cube", m_Rotation.values);
			ImGui::DragFloat3("Position##Cube", m_Position.values, 0.01f, -1.0f, 1.0f);
		}
		if (ImGui::CollapsingHeader("Camera"))
			ImGui::DragFloat3("Position##Camera", m_CameraPosition.values, 0.01f, -5.0f, 5.0f);
		ImGui::End();
	}

	void Camera::OnWindowResize(const FW::Event::WindowResizeEvent& event)
	{
